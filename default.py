...
class DefaultStrategy(Strategy):
    ...
    def step_post_backward(
        ...
        if (
            ...
            if self.dropout > 0.0:
                to_keep = 1.0 - self.dropout
                mask = torch.rand(n_gaussian) < to_keep
            else:
            ...
            grow_num = n_dupli + n_split
            if grow_num > 0:
                mask = torch.cat([mask, torch.zeros(grow_num, device=device).bool()])

            # prune GSs
            n_prune = self._prune_gs(params, optimizers, state, step, mask)
            if self.verbose:
                print(
    ...
    def _grow_gs(
        ...
        is_dupli = is_grad_high & is_small & mask
    ...
    def _prune_gs(
        ...
        if step > self.reset_every:
            ...
            if step < self.refine_scale2d_stop_iter:
                is_too_big |= state["radii"][mask] > self.prune_scale2d
